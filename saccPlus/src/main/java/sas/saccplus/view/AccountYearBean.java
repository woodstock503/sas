...
import sas.saccplus.session.TransactionDetailFacade;
import sas.saccplus.util.ConverterDTO;
import sas.saccplus.util.SessionBean;
import sas.saccplus.util.StringUtil;
...
public class AccountYearBean implements Serializable
{
	...
	public void closeAccountYear(){
		try{
			...
			AccountYear nextAccountYear = accountYearFacade.findAccountYear(nextAccountYearDate.getTime(),session.getCompany(),session.getUser());
			...
			accountYear.setUpdateBy(session.getUser().getUserName());
	...
	public String update()
	{
		...
		try
		{
			if (this.id == null)
			{
				...
				accountYear.setCreateBy(session.getUser().getUserName());
